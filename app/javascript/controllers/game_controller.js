...
export default class extends Controller {
  ...
  connect() {
    ...
    const LAST = this.lastScoreTarget
    const PERCENTAGE = this.percentageTarget
    ...
    window.addEventListener("message", (event) => {
      ...
      if (event.data.type === "score") {
        ...
        this.lastScoreValue = event.data.value
        LAST.value = this.lastScoreValue
      }

      if (event.data.type === "bestScore") {
        // This is the BEST score
        this.bestScoreValue = event.data.value
        SCORE.value = this.bestScoreValue
      }

      if (this.bestScoreValue && this.lastScoreValue) {
